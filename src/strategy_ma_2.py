...
class StrategyMa2(StrategyBase):
    ...
    def before_run(self, df: pd.DataFrame):
        cond_signal_open = (df.ma_s > df.ma_l) & (df.ma_s.shift(1) < df.ma_l.shift(1))
        df.loc[cond_signal_open, ['signal']] = 'open'

        cond_signal_close = (df.ma_s < df.ma_l) & (df.ma_s.shift(1) > df.ma_l.shift(1))
